...
const AI_PROMPT = 'For the position title: {positionTitle}, give me 5-7 bullet points for my experience in resume. Return the result as a JSON object with "position_title" and "experience" fields. The "experience" field should be an array of strings, each representing a bullet point.';

function ExperienceForm({ enableNext }) {
  const { resumeInfo, setResumeInfo } = useContext(ResumeInfoContext);
  const params = useParams();
  const [experienceList, setExperienceList] = useState(
    resumeInfo.experience?.length > 0 ? resumeInfo.experience : [INITIAL_FORM_FIELD]
  );
  ...
  const handleRichTextChange = useCallback((value, index) => {
    setExperienceList(prevList => 
      prevList.map((item, i) => i === index ? { ...item, workSummary: value } : item)
    );
  }, []);
  ...
  const isValidExperience = useCallback((experience) => {
    return Object.values(experience).every(value => {
      // Check if the value is falsy or not a string before calling .trim()
      return typeof value === 'string' && value.trim() !== "";
    });
  }, []);
  

  const GenerateSummaryFromAI = useCallback(async (index) => {
    enableNext(false);
    setLoading(true);
    ...
    try {
      ...
      setExperienceList(prevList => 
        prevList.map((item, i) => i === index ? { ...item, workSummary: formattedResponse } : item)
      );
    } catch (error) {
  ...
  const onSave = useCallback(async (e) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.error?.message || "Failed to update details");
    } finally {
      setLoading(false);
    }
  }, [params.resumeId, experienceList, isValidExperience]);
  ...
  return (
    <div className="p-5 shadow-lg rounded-lg border-t-primary border-t-4 mt-10">
      ...
      {experienceList.map((formData, index) => (
        <div key={index} className="mt-5 gap-3 border-2 p-4 rounded-lg shadow-md">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="col-span-2">
              <div className="flex justify-between items-center mt-4">
                ...
                >
                  <Brain />
                  {loading ? <LoaderCircle className="h-4 w-4 animate-spin" /> : "Generate from AI"}
                </Button>
              </div>
              <ReactQuill
                className="mt-2"
                ...
                formats={["header", "font", "size", "bold", "italic", "underline", "list", "bullet", "indent", "link"]}
      ...
      <div className="flex justify-between gap-4 mt-4">
        <div className="flex gap-4">
          <Button variant="outline" onClick={addNewExp}>+ Add More Experience</Button>
          <Button onClick={removeExp} disabled={experienceList.length <= 1}>- Remove</Button>
        </div>
